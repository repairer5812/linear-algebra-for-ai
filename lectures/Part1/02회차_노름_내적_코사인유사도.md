...
**직관 (세 Norm의 비교)**: 같은 Vector $\mathbf{x} = (3, 1, 2, 4) \in \mathbb{R}^4$도 Norm 종류에 따라 크기 값이 달라진다.
...
**평면(2D)에서의 사실** (Strang §1.2). 평면의 임의의 Vector는 길이와 방향만으로 결정된다. 길이 $\|\mathbf{u}\|$, x축과 이루는 각 $\alpha$인 Vector의 성분은 삼각비에서

$$\mathbf{u} = (\|\mathbf{u}\|\cos\alpha,\ \|\mathbf{u}\|\sin\alpha)$$

이다 (극좌표 표현). 두 Vector를 각각 각 $\alpha, \beta$로 나타내면 Dot product 계산에서 두 각의 차 $\alpha - \beta$가 직접 나타나고, 이것이 두 Vector 사이의 각 $\theta$이다.

![w:440](assets/02_c3_angle.png)
...
→ **2차원·3차원의 기하적 사실**이다. 같은 결과를 코사인 법칙으로도 유도한다 (다음 슬라이드).

---

## C-3. 코사인 법칙에 의한 유도

원점 O와 두 Vector의 끝점이 이루는 삼각형에서 세 변의 길이는 $\|\mathbf{u}\|,\ \|\mathbf{v}\|,\ \|\mathbf{u}-\mathbf{v}\|$이고, $\mathbf{u}, \mathbf{v}$ 사이 각이 $\theta$이다. 코사인 법칙은
...
### 문제 1 (계산)
$\mathbf{u} = (1, 1, 1)$, $\mathbf{v} = (1, -1, 0)$에 대해:
