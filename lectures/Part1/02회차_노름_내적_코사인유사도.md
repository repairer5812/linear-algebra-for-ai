...
본 강의에서 별도 표기가 없으면 $\|\mathbf{x}\| = \|\mathbf{x}\|_2$이다.
...
→ (N3) 삼각부등식은 **Cauchy-Schwarz의 결과**로 C 섹션에서 증명한다.
...
는 $\|\hat{\mathbf{x}}\|_2 = 1$인 **단위 Vector**(unit vector)이다. (N2) 동차성의 직접 응용이다.
...
> **힌트**: (N2) 동차성 $\|\alpha\mathbf{x}\| = |\alpha|\,\|\mathbf{x}\|$ 검사가 가장 빠른 판별이다.
...
- **(ii) $h$는 Norm이다**. 가중치 다른 $\ell_1$ 변형. (N1) $\sum a_i|x_i| \ge 0$ ✓, (N2) $h(\alpha\mathbf{x}) = |\alpha|\,h(\mathbf{x})$ ✓, (N3) 삼각부등식 ✓.
...
## C-5. Cauchy-Schwarz의 두 결과: 삼각부등식과 각도
...
→ **방향만 보고 크기는 무시한다**. BERT·CLIP 같은 Embedding 모델 출력 비교의 자연스러운 도구이다.
...
2. **Dot product**는 두 Vector의 관계를 한 Scalar로 묶는다. $\mathbf{u}\cdot\mathbf{u} = \|\mathbf{u}\|_2^2$이므로 $\ell_2$가 Dot product에서 자동 유도된다.
3. **각도는 정의**이다. $\cos\theta := \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}$, 2D·3D 기하 사실의 $n$차원 확장.
4. **Cauchy-Schwarz** ($|\mathbf{u}\cdot\mathbf{v}| \le \|\mathbf{u}\|\|\mathbf{v}\|$)가 $\cos\theta \in [-1, 1]$과 $\ell_2$ 삼각부등식을 동시에 보장한다. 증명 = **이차식 비음성 → 판별식** 패턴.
