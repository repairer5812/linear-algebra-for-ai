...
MML §3.1-§3.4 (메인) · Strang Ch 1.2 (발췌, Cauchy-Schwarz 판별식 풀이) · EoLA Ch.9 · Part 1 (LA1)
...
> 📚 **Strang Ch 1.2 발췌 (시그니처)**: Strang은 $\|\mathbf{u} - t\mathbf{v}\|^2 \ge 0$이 $t$에 대한 이차식이라는 한 줄에서 **판별식 조건**으로 Cauchy-Schwarz를 한 페이지에 풀이한다. 본 회차의 증명 흐름이 이 발췌의 직역이다 (이차식 비음성 → 판별식 $\le 0$). 이 패턴은 학기 전반의 정리 증명에서 반복된다.
...
**평행하면 그림자가 본래 막대와 일치합니다** (등호). Part 2 2회차 정사영의 출발점입니다.
...
**반례**: $\mathbf{e}_1 \cdot \mathbf{e}_2 = 0$인데 둘 다 영Vector가 아니다. **Orthogonal**이라는 새 관계이다 (Part 2 1회차 본격 정의).
...
→ **방향만 보고 크기는 무시한다**. BERT(문장 임베딩 언어 모델)·CLIP(이미지·텍스트 공동 임베딩 모델) 같은 Embedding 모델 출력 비교의 자연스러운 도구이다 (정식 분해는 Part 4).
...
## D-4. 미리보기: Attention과의 연결 (Part 4 7회차)

Transformer Attention(어텐션) 한 줄:

> 표기 안내 (정식 도입은 Part 4 7회차):
...
→ **본 회차에서 본 Dot product와 Norm이 곧 LLM의 핵심 연산**이다. Part 4 7회차에서 본격 다룬다.
...
**사전 reading**:
- **MML §2.2 (Matrices)** + **§2.7 (Linear Mappings 도입)**, 메인
- **Strang Ch 1.3·2.1 발췌** (Row picture vs Column picture, 시그니처), 본문 박스로 가져옴
