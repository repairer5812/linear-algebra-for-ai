...
> **(a)** MNIST 숫자 "7" 이미지 $n$장 $\mathbf{x}_1, \ldots, \mathbf{x}_n$의 평균 이미지 $\mathbf{m}$을 Vector·Linear combination 두 단어를 모두 써서 한 줄로 표기.
> **(b)** Self-attention $\mathrm{softmax}(QK^\top/\sqrt{d_k})V$의 5층 건물 매핑 중 두 층.
...
- **(b)** 5층 건물 매핑 예시 (둘 중 어느 쪽이든 가능):
   - **Vector·Matrix 곱**: $QK^\top$, $\mathrm{softmax}(\cdot)\,V$가 모두 Matrix·Vector 곱(또는 Matrix·Matrix 곱). 3회차의 주제.
...
- **(c)** **Part 1**: 12회차, **Part 2**: 9회차, **Part 3**: 9회차 (합 30회차). **1회차당 2시간**.

### 핵심 관찰

(a)의 한 줄은 오늘 본 회차에서 정식화될 모든 것을 담고 있습니다, $\mathbb{R}^{784}$ Vector 정의, 두 연산(덧셈·Scalar곱), 그리고 그 둘을 합친 **Linear combination**. 평균 이미지가 다시 이미지인 이유는 "Vector를 Linear combination해도 Vector이다"라는 본 회차 핵심 사실입니다.

→ 오늘은 그 토대인 **Vector·$\mathbb{R}^n$·Linear combination**을 정식으로 도입합니다. 평균 Vector $\mathbf{m}$의 정의에 이미 오늘 배울 모든 것이 들어 있습니다.
...
이 성질들을 한 데 묶어 **Vector space의 8 axioms(공리)**이라 부릅니다. **정식 정의는 6회차에서 도입**합니다. 그때까지는 $\mathbb{R}^n$이 위 성질을 모두 만족한다는 사실만 받아들입니다.

> **왜 미루는가?** 1~5회차는 $\mathbb{R}^n$이라는 **친숙한 공간**에서 충분히 손계산·시각화를 한 뒤, 6회차에서 "이 모든 성질을 가진 객체를 일반적으로 Vector space라 부르자"는 추상화로 자연스럽게 넘어가는 것이 학부 미이수자에게 가장 큰 부담이 적습니다.
...
계수 $\alpha_i$가 **양수·음수·0** 어느 것이든 가능합니다. 모두 0이면 결과는 영Vector $\mathbf{0}$.
...
> **AI 모델의 비선형성은?** 신경망의 **활성화 함수**(ReLU·sigmoid 등)가 비선형 부분을 담당합니다. 가중치 곱·합은 linear, 활성화는 비선형, **두 부분이 분리**되어 있어 각각을 따로 분석할 수 있습니다 (5·10회차). 예: ReLU는 $\max(0, x)$, sigmoid는 $1/(1+e^{-x})$.
...
### 핵심 관찰
**Vector 개수 ≠ Span의 차원**입니다. $(1,1)^\top$과 $(2,2)^\top$는 두 개지만 Span은 1차원 직선. **"진짜 다른 방향"이 몇 개인가**가 차원입니다, 8회차에서 **Linear independence**로 정식화합니다.
...
> **Basis·Dimension의 정식 정의는 8회차**입니다. 오늘은 "$\mathbf{e}_1, \ldots, \mathbf{e}_n$로 $\mathbb{R}^n$ 전체를 만들 수 있다"는 직관만 챙깁니다.
...
> **메시지**: Vector 개수 = Span 차원이 아닙니다. **"진짜 다른 방향" 개수**가 차원입니다. 8회차에서 **Linear independence**로 정식화합니다.
...
> $\mathbf{b}$는 Bias(편향): 출력을 평행이동시키는 상수 Vector입니다 (5·10회차에서 본격).
...
> **핵심**: 세 Vector 중 하나가 나머지의 Linear combination이면 Span에 기여하지 않습니다, **"진짜 다른 방향" 개수가 차원**입니다. 8회차 Linear independence의 핵심 직관.
