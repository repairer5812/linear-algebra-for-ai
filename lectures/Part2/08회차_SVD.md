...
**기하적 해석 (SVD의 보편성)**: SVD $A = U\Sigma V^\top$는 임의의 행렬 $A$의 선형변환을 세 단계로 분해한다, (1) $V^\top$, 입력 공간에서의 직교 회전 (입력 공간의 정규직교 기저를 표준 기저로 보냄), (2) $\Sigma$, 각 축별 $\sigma_i$배 신축 (그리고 차원 변환), (3) $U$, 출력 공간에서의 직교 회전. 회전과 신축이 가장 단순한 두 기본 작용이므로, **임의의 선형변환이 세 단계의 합성으로 분해된다는 사실**이 SVD가 모든 행렬에 적용되는 일반성을 갖는 이유이다.
...
본 회차 학습 흐름 (SVD 정의 → $\sigma_i^2$ → 4 부분공간 → 기하)을 한 문제로 종합합니다.
