...
**직관 (잠재변수 추정의 의미)**: GMM에서 각 표본 $\mathbf{x}_n$은 어느 컴포넌트 $k$에서 생성되었는지 모르는 채 관측된다. 컴포넌트 라벨 $z_n \in \{1, \ldots, K\}$이 **잠재변수**이다. 라벨을 안다면 컴포넌트별 평균·분산을 표본 평균·분산으로 직접 풀 수 있지만, 모르는 채로는 likelihood가 닫힌 형태로 풀리지 않는다. EM은 (E) 현재 파라미터로 라벨의 posterior를 계산, (M) posterior로 가중평균해 파라미터를 갱신, 두 단계의 반복으로 해를 구한다.
