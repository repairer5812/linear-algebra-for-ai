...
> 본 회차는 **평가가 아니라 학습 도구**다. 점수 환산 없음. 본 강좌 전체의 종합 활동.
...
| Part 3 도구 7개 통합? | **종합 문제 풀이** | Part 3 1-7회차 |
...
| ③ | **C** | **Part 3 종합 문제 풀기** (Regression → PCA → GMM → SVM → CNN → Attention) |
...
### 목적
본 강좌 7개 회차 (Part 3) 동안 쌓은 도구로 **임의의 AI 모델 한 부분**을 LA 객체로 환원해 본다. **학습 도구**이며 평가하지 않는다.
...
## B-3. 발표 채점 X·학습 도구 명시

> 본 회차는 **점수 환산이 없다**. 발표는 평가 도구가 아니라 **본 강좌 전체를 자신의 손으로 정리하는 학습 활동**이다.
...
4. **Case Study는 학습 도구**이며 평가가 아니다. 학생 각자가 본 강좌 도구를 자기 언어로 정리한다.
