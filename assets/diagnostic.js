...
const CASE_TEXT = {
  A: {
    headline: "본 강좌는 귀하에게 복습용입니다.",
    body: `
      <p>진단 결과 6축 중 대부분에서 70% 이상을 기록하셨습니다. 본 강좌는 학부 선형대수의 표준 분량을 차분히 다루는 흐름으로 설계되어 있어, 귀하 수준에서는 Part 1 전반(W1-W10)이 복습에 가깝습니다.</p>
      <p>다음을 권합니다.</p>
      <ul>
        <li><strong>더 심화된 주제</strong>: 텐서대수, 미분기하, 정보이론, 무한차원 함수해석, 작용소이론 등 후속 주제가 본인 연구와 더 직접적으로 연결될 수 있습니다.</li>
        <li><strong>본 강좌 청강은 환영</strong>: 다만 출석·과제 부담을 새로 지기보다, 다음 두 부분만 선별 청강하는 방식이 효율적입니다.
          <ul>
      ...
      <p>본 강좌의 목적이 "정의·정리에서 출발해 AI 모듈 분해까지"인 만큼, 이미 정의·정리가 익숙하다면 응용 후반부에 집중하는 것이 가장 효율적입니다.</p>
    `
  },
  B: {
    headline: "본 강좌가 적합합니다. 약점 축 위주로 학습하시면 효율적입니다.",
    body: `
      <p>진단 결과 일부 축은 견고하나 다른 축에서 50-70% 범위로 떨어집니다. 본 강좌는 6축을 모두 다루도록 설계되어 있어 귀하의 약점 축 보완에 직접적인 도움이 됩니다.</p>
      <ul>
        <li><strong>강점 축</strong>: 해당 주차는 빠르게 통과하며 응용에 집중</li>
        <li><strong>약점 축</strong>: 해당 주차의 사전 reading(MML 본문 + EoLA 시각 자료)을 1-2주 먼저 읽어두면 강의가 훨씬 수월합니다.</li>
      </ul>
      <p><strong>축별 사전 reading 가이드</strong> (약점 축에 해당하는 행만 우선 학습):</p>
      ...
      <p><strong>이 강좌에서 무엇을 얻는가</strong>: 약점 축의 정의·정리를 정확히 진술 가능한 수준까지 끌어올리고, 강점 축은 AI 응용 맥락에서 재해석되어 본인의 연구·코드 작성 시 LA 객체 식별 속도가 빨라집니다.</p>
    `
  },
  C: {
    headline: "본 강좌의 표준 대상입니다. Part 1부터 차근차근 따라가시면 됩니다.",
    body: `
      <p>진단 결과 6축 중 다수에서 50% 미만을 기록하셨습니다. 본 강좌는 정확히 이 출발점을 가정하고 설계되었습니다. 정의에서 출발해 정리·증명·코딩 실습을 매 주차 반복하므로, 학기를 따라가면 자연스럽게 6축 전체가 채워집니다.</p>
      <p><strong>학습 부담 안내</strong> (현실적인 시간 추정):</p>
      <ul>
        <li>매 주 강의 100분 + 과제 4-6시간 + reading 2-3시간 = <strong>주당 약 8-10시간</strong></li>
        <li>시험 직전 주: +5-10시간</li>
        <li>Part 2 텀 프로젝트(W12-W16): 추가 20-30시간</li>
        <li>학기 총 부하 추정: Part 1 약 150시간, Part 2 약 180시간 (대학원 3-4학점 표준)</li>
      </ul>
      <p><strong>이 강좌에서 무엇을 얻는가</strong>:</p>
      <ul>
      ...
      <p>처음에 어렵게 느껴지더라도 매 주차 자가진단 체크리스트로 진도를 점검하면 막힘 없이 따라올 수 있도록 설계되어 있습니다.</p>
    `
  },
  D: {
    headline: "특정 영역 깊이는 좋으나 다른 영역이 부족합니다. 본 강좌의 통합 흐름이 도움될 수 있습니다.",
    body: `
      ...
      <p><strong>이 강좌에서 무엇을 얻는가</strong>: 6축이 별개 영역이 아니라 하나의 큰 그림 — "$Ax=b$의 해 → 부분공간 분류 → 직교 분해 → 고유 분해 → SVD → AI 모듈 환원"이라는 단일 흐름 — 임을 학기 내내 반복적으로 보게 됩니다. 강점 축은 더 깊어지고, 약점 축은 강점 축과의 연결 속에서 빠르게 채워집니다.</p>
    `
  }
};
...
function reset() {
  if (!confirm("진단 결과와 응답을 모두 삭제하고 다시 시작합니다. 진행할까요?")) return;
